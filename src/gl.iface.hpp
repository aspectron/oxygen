...
//					glUseProgram(m_program);
		...
		gui::window *get_pwnd(void) const { return m_pwnd; }
...
//	gui::window *get_pwnd_wingui(void) { return (gui::window*)get_pwnd(); }
...
public:
	...
	void cleanup(void)
	{
		...
		if(m_hdc) ::ReleaseDC(*get_pwnd(),m_hdc);
	}


	// 				void get_viewport_size(int *pwidth, int *pheight)
	// 				{
	// 					get_pwnd_generic()->get_size(pwidth,pheight);
	// 				}
	...
	void output_text(double x, double y, wchar_t *text)
	{
		...
		int width,height;
		get_pwnd()->get_size(&width,&height);
...
class iface : public iface_base
{
	...
	private:
		...
		xf86gui::window *get_pwnd_xf86gui(void) { return (xf86gui::window*)get_pwnd(); }
	...
	public:
		...
		iface(gui::window *pwnd)
			: iface_base(pwnd),
				m_glx_context(NULL)
		{
			m_window = get_pwnd_xf86gui()->get_window();
		}

		bool setup(void)
		{

			m_glx_context = glXCreateContext(xf86gui::g_display, &get_pwnd_xf86gui()->get_current_visual(), glXGetCurrentContext(), true);
...
printf("extensions: %s",extensions);
		...
		void swap_buffers(void)
		{
			if (get_pwnd_xf86gui()->get_window() && m_glx_context)
				glXSwapBuffers(xf86gui::g_display, get_pwnd_xf86gui()->get_window());
